...
* The persistence structure is designed to make `bot_data`, `chat_data`, `user_data` and `ConversationHandler`'s states persistent.<br>
Note that `Handler`'s with `run_async=True` are not supported because it uses `threading.Lock` which can't be serialized
